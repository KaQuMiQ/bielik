...
install:
	...
	@$(UV_ALIAS) pip install $(INSTALL_OPTIONS) --system --constraint constraints
	@echo '...finished!'
...
venv:
	...
	@. ./.venv/bin/activate && pip install --upgrade pip && $(UV_ALIAS) pip install --editable $(INSTALL_OPTIONS) --constraint constraints
...
sync:
	...
	@$(if $(findstring $(UV_VERSION), $(shell $(UV_ALIAS) --version | head -n1 | cut -d" " -f2)), , @echo '...updating uv...' && curl -LsSf https://github.com/astral-sh/uv/releases/download/$(UV_VERSION)/uv-installer.sh | sh)
...
run:
	chainlit run ./src/entrypoint/chat.py
